...
import {RequestEmployeeCardFormGroup} from '../../models/form-builder/requestEmployeeCard.formBuilder';
...
@Component({
    selector: 'app-request-employee-card',
    templateUrl: './request-employee-card.component.html',
    styleUrls: ['./request-employee-card.component.scss']
})

export class RequestEmployeeCardComponent {
    formGroup: RequestEmployeeCardFormGroup;
    ...
    constructor(public dataSource: RestDataSource, private ref: ChangeDetectorRef, 
                public imageUtility: ImageUtility,
                private snackBar: MatSnackBar) {
        this.formGroup = new RequestEmployeeCardFormGroup();
    ...
    get titleForm(): string {
        return 'فرم ثبت درخواست کارت شاغلین';
    }
        ...
        if (form.valid) {
            ...
            this.dataSource.getEmployee(this.empNoInput).subscribe(data => {
                ...
                if (data.img != null && data.img.length !== 0) {
                    this.imageUtility.convertBlob_2_B64(this.imageUtility.convertNumber_2_Blob(data.img),
                        (imgDataParam) => {
    ...
    submitForm(form: NgForm) {
        ...
        if (this.formGroup.valid) {
            ...
            const DESC = null;
            const REPLICA = this.formGroup.controls['duplicateCard'].value;
            ...
            const GEOWORK = this.formGroup.controls['geoWork'].value;
            const STATUS = 1;
            const creatorid = user['userId'];
            const editorid = user['userId']; 
            ...
            const REPLICA_IN_REQUEST = this.formGroup.controls['duplicateCard'].value;
            ...
            const EMPLOYEE_TYPE = this.formGroup.controls['typeCard'].value;
            ...
            const CODE_PA_OM = +this.employee.panish; 
            const OMOR_TITLE = 0;
